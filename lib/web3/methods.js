        ...
        try {
            ...
            solcWorker.on('message', m => {
                if(m.compiled) {
                    ...
                    sources[m.sources.cleanURL] = { content: m.sources.content }
                    const input = { language: 'Solidity', sources, settings };
                    solcWorker.send({ command: 'compile', payload: input });
                } else if (m.path) {
                    ...
                    const localFSHandler = [
                        {
                            ...
                            handle: (match) => { return this.handleLocal(match[2], match[3]) }
                        }
                    ]
                    urlResolver.resolve(m.path, localFSHandler)
                        .then(_sources => {
                            sources[_sources.cleanURL] = { content: _sources.content }
                            const input = { language: 'Solidity', sources, settings };
                            solcWorker.send({ command: 'compile', payload: input });
                        })
                        .catch(e => {
